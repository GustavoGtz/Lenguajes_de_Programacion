    ...
    (cond
      [(and (not (eof-object? ch))(char-digit? ch))
       ...
       (let* ([tokens (lex-plain-number chars)]
         ...
         (stream-cons token (stream-rest tokens)))]
      [else
       (signal-lex-error "minus operation not supported, expected a number" lex-line col)])))
    ...
    (cond
      [(char-delimiter? ch)
       (let ([token (token-binop '*)])
         ...
         (stream-cons token (lex chars)))]
      [else
       (signal-lex-error "expected delimiter after multiplication" lex-line lex-col)])))
...
(define (signal-lex-error message line col)
  (error 'lex (format "lexical error at ~a:~a:~a: ~a" lex-path line col message)))
